...
pagefault(uint64 va)
{
  ...
  if (va > p->sz)
  {
    printf("pagefault error: address was not previously allocated by sbrk");
    kill(p->pid);
    return;
  }
  if (walkaddr(pagetable, va) != 0)
  {
    printf("pagefault error: virtual address was already mapped");
    kill(p->pid);
    return;
  }
  ...
  char *mem;
  if ((mem = kalloc()) == 0){
    printf("pagefault error: kalloc() couldn't allocate memory");
    kill(p->pid);
    return;
  }
